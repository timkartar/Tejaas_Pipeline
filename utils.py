...
def load_matrixeqtl_results(chr_list, gtex_dir, cardio_dir):
    results_file = os.path.join(gtex_dir, "gtex_MatrixEQTL.transout")
    gtex_matrix_dict = {}
    with open(results_file) as instream:
        ...
        for line in instream:
            ...
            if rsid not in gtex_matrix_dict:
                gtex_matrix_dict[rsid] = FDR
                
    results_file = os.path.join(cardio_dir, "cardiogenics_MatrixEQTL.transout")
    cardio_matrix_dict = {}
    with open(results_file) as instream:
        _ = instream.readline()
        for line in instream:
